...
import { semi2Countries, APP_CONFIG, VOTING_PHASES } from '../data/eurovision2026'
...
const STAGE = VOTING_PHASES.SEMI_2

export default function SemiFinal2() {
  const [activeView, setActiveView] = useState('Running Order')
  const { currentUser, isAuthenticated } = useAuth()
  const { myVotes, castVote, saving, isReady } = useVoting(STAGE)
  const leaderboard = useLeaderboard(STAGE, semi2Countries)

  const totalPointsGiven = Object.values(myVotes).reduce((s, p) => s + p, 0)
  const countriesVoted = Object.keys(myVotes).length

  return (
    <PageContainer
      badge="Semi Final 2"
      title="Second Semi Final"
      subtitle={`${APP_CONFIG.dates.semi2} · Голосування за країни, що пройдуть до Grand Final`}
    >

{/* Live Participants panel */}
      <LiveParticipants stage={STAGE} accent="cyan" />

      {/* Status banner */}
      {isAuthenticated ? (
        <div className="flex items-center gap-3 bg-neon-cyan/8 border border-neon-cyan/25 rounded-xl px-4 py-3 mb-6">
          <div className="w-2 h-2 rounded-full bg-neon-cyan animate-pulse" />
          <p className="text-white/70 text-sm font-body flex-1">
            <span className="text-neon-cyan font-semibold">Voting Open</span>
        ...
        <div className="flex items-center gap-3 bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 mb-6">
          ...
          <p className="text-white/50 text-sm font-body flex-1">
            ...
            {' · '}
            <Link to="/login" className="text-neon-cyan hover:text-neon-cyan/80 underline underline-offset-2 transition-colors">
              Увійти через Google
      ...
      {activeView === 'Running Order' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SectionCard
            title="Running Order"
            subtitle={`${semi2Countries.length} країн · Semi Final 2`}
            accent="cyan"
          >
            <div className="space-y-2 mt-4">
              {semi2Countries.map((c, i) => (
                <React.Fragment key={c.id}>
                  <CountryCardPlaceholder {...c} score={myVotes[c.id] ?? undefined} variant="default" />
                  ...
                  {c.id === 'cz' && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-neon-violet/5 border border-neon-violet/15">
                      ...
                      <span className="text-xs font-mono text-white/30">France — guest performance (auto-qualified)</span>
                    </div>
                  )}
                  {c.id === 'cy' && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-neon-violet/5 border border-neon-violet/15">
                      <span className="text-sm">🇦🇹</span>
                      <span className="text-xs font-mono text-white/30">Austria — guest performance (host, auto-qualified)</span>
                    </div>
                  )}
                  {c.id === 'ua' && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-neon-violet/5 border border-neon-violet/15">
                      <span className="text-sm">🇬🇧</span>
                      <span className="text-xs font-mono text-white/30">United Kingdom — guest performance (auto-qualified)</span>
                    </div>
                  )}
                </React.Fragment>
              ))}
          ...
          <div className="space-y-4">
            {/* Info card */}
            <SectionCard title="Як це працює?" accent="blue">
              <div className="space-y-3 mt-3 text-sm text-white/50 font-body leading-relaxed">
                <p>🎵 Другий Semi Final — ще один шанс побачити улюблені країни на сцені.</p>
                <p>⭐ Натискай на бал від 1 до 12 під кожною країною. Можна змінити будь-коли.</p>
                <p>🏆 Leaderboard оновлюється в реальному часі для всіх учасників.</p>
                {isAuthenticated
                  ? <p className="text-neon-cyan/80 font-medium">✅ Ти увійшов — голосування відкрито!</p>
                  : <p className="text-white/30"><Link to="/login" className="text-neon-cyan/70 hover:text-neon-cyan underline">Увійди</Link> щоб проголосувати</p>
                }
              </div>
            </SectionCard>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        i < 3 && c.totalPoints > 0 && 'bg-neon-cyan/[0.04] border-neon-cyan/10'
                      )}
                    >
                      ...
                      <span className={cn(
                        ...
                        c.totalPoints > 0 ? 'text-neon-cyan' : 'text-white/15'
                      )}>
            ...
            <div className="flex items-center gap-4 bg-white/[0.04] border border-white/[0.07] rounded-xl px-4 py-3 mb-5">
              ...
              <div className="flex-1 flex flex-wrap gap-1.5">
                {POINT_VALUES.map(pts => {
                  const usedBy = Object.entries(myVotes).find(([, p]) => p === pts)
                  const countryData = usedBy ? semi2Countries.find(c => c.id === usedBy[0]) : null
                  return (
              ...
              <span className="text-white/30 text-xs font-mono shrink-0">{countriesVoted}/14</span>
            </div>
          )}

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {semi2Countries.map(c => {
...
function VotingCard({ country: c, selectedPoints, isSaving, isAuthenticated, isReady, onVote, accent }) {
  return (
    <div className={cn(
      ...
      selectedPoints && accent === 'cyan' && 'border-neon-cyan/30',
      selectedPoints && accent === 'pink' && 'border-neon-pink/30',
      ...
      <div className="text-center">
        {isSaving ? (
          ...
          <Link to="/login" className={cn(
            'text-white/25 text-xs font-mono transition-colors',
            accent === 'cyan' ? 'hover:text-neon-cyan' : 'hover:text-neon-pink'
          )}>
            Sign in to vote ↗
          </Link>
        ) : selectedPoints ? (
          <span className={cn(
            ...
            accent === 'cyan' ? 'text-neon-cyan/60' : 'text-neon-pink/60'
          )}>
